...
class Wilayah extends Backend_Controller {
	...
	public function __construct() {
		parent::__construct();
		$this->dir = base_url( 'rekap/wilayah/' );
	}

	function index() {
		$data = array();
		$data['legend'] = $this->db->get('dbo.ref_references')->result_array();
		$data['title'] = 'Rekapitulasi Wilayah';
		...
		$this->template->title( 'Rekapitulasi Wilayah' );
		$this->template->content( "admin/rekap/wilayah", $data );
	...
	function get_rekap_wilayah(){
		...
		$query = "SELECT ".$select."
					...
					( SELECT COUNT ( * ) FROM asset.master_data_proses A WHERE ".$where_status0.") AS status_0,
					SUM ( CASE WHEN (( md.stereotype ) IN ( 'PROVINCE-PUBLISHED' )) THEN 1 ELSE 0 END ) AS status_1,
					SUM ( CASE WHEN (( md.stereotype ) IN ( 'MUSDES-PUBLISHED' )) THEN 1 ELSE 0 END ) AS status_2,
					SUM ( CASE WHEN ( md.stereotype IN ( 'MUSDES-REVOKED' )) THEN 1 ELSE 0 END ) AS status_2a,
					SUM ( CASE WHEN ( md.stereotype IN ( 'MUSDES-GRABBED' )) THEN 1 ELSE 0 END ) AS status_3,
					SUM ( CASE WHEN (( md.stereotype ) IN ( 'MUSDES-GRAB-REVOKED' )) THEN 1 ELSE 0 END ) AS status_3a,
					SUM ( CASE WHEN (( md.stereotype ) IN ( 'MUSDES-DOWNLOADED' )) THEN 1 ELSE 0 END ) AS status_4,
					SUM ( CASE WHEN (( md.stereotype ) IN ( 'MUSDES-DOWNLOAD-REVOKED' )) THEN 1 ELSE 0 END ) AS status_4a,
					SUM ( CASE WHEN ( md.stereotype IN ( 'MUSDES-SURVEY' )) THEN 1 ELSE 0 END ) AS status_5,
					SUM ( CASE WHEN ( md.stereotype IN ( 'MUSDES-SURVEY-REVOKED' )) THEN 1 ELSE 0 END ) AS status_5a,
					SUM ( CASE WHEN (( md.stereotype ) IN ( 'MUSDES-SUBMITTED' )) THEN 1 ELSE 0 END ) AS status_6,
					SUM ( CASE WHEN (( md.stereotype ) IN ( 'MUSDES-NOT-FOUND' )) THEN 1 ELSE 0 END ) AS status_6a,
					SUM ( CASE WHEN (( md.stereotype ) IN ( 'VERIVALI-PUBLISHED' )) THEN 1 ELSE 0 END ) AS status_7,
					SUM ( CASE WHEN ( md.stereotype IN ( 'VERIVALI-REVOKED' )) THEN 1 ELSE 0 END ) AS status_7a,
					SUM ( CASE WHEN ( md.stereotype IN ( 'VERIVALI-GRABBED' )) THEN 1 ELSE 0 END ) AS status_8,
					SUM ( CASE WHEN (( md.stereotype ) IN ( 'VERIVALI-GRAB-REVOKED' )) THEN 1 ELSE 0 END ) AS status_8a,
					SUM ( CASE WHEN (( md.stereotype ) IN ( 'VERIVALI-DOWNLOADED' )) THEN 1 ELSE 0 END ) AS status_9,
					SUM ( CASE WHEN ( md.stereotype IN ( 'VERIVALI-DOWNLOADED-REVOKED' )) THEN 1 ELSE 0 END ) AS status_9a,
					SUM ( CASE WHEN ( md.stereotype IN ( 'VERIVALI-SURVEY' )) THEN 1 ELSE 0 END ) AS status_10,
					SUM ( CASE WHEN (( md.stereotype ) IN ( 'VERIVALI-SURVEY-REVOKED' )) THEN 1 ELSE 0 END ) AS status_10a,
					SUM ( CASE WHEN (( md.stereotype ) IN ( 'VERIVALI-SUBMITTED' )) THEN 1 ELSE 0 END ) AS status_11,
					SUM ( CASE WHEN (( md.stereotype ) IN ( 'VERIVALI-SUBMITTED-REJECT' )) THEN 1 ELSE 0 END ) AS status_11a,
					SUM ( CASE WHEN ( md.stereotype IN ( 'VERIVALI-SUPERVISOR-APPROVED' )) THEN 1 ELSE 0 END ) AS status_12,
					SUM ( CASE WHEN ( md.stereotype IN ( 'VERIVALI-KORKAB-REJECTED' )) THEN 1 ELSE 0 END ) AS status_12a,
					SUM ( CASE WHEN ( md.stereotype IN ( 'VERIVALI-KORKAB-APPROVED' )) THEN 1 ELSE 0 END ) AS status_13,
					SUM ( CASE WHEN ( md.stereotype IN ( 'VERIVALI-KORWIL-REJECTED' )) THEN 1 ELSE 0 END ) AS status_13a,
					SUM ( CASE WHEN ( md.stereotype IN ( 'VERIVALI-PENDING-QC-PUSAT' )) THEN 1 ELSE 0 END ) AS status_13b,
					SUM ( CASE WHEN ( md.stereotype IN ( 'VERIVALI-FINAL' )) THEN 1 ELSE 0 END ) AS status_14,
					SUM ( CASE WHEN ( md.stereotype IN ( 'SYNC-SIKS-NG' )) THEN 1 ELSE 0 END ) AS status_15
				FROM
					[asset].[master_data_proses] md
					RIGHT JOIN [dbo].[target_desa] l ON (
