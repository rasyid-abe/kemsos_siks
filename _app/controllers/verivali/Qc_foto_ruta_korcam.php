...
class Qc_foto_ruta_korcam extends Backend_Controller {
	public function __construct() {
		parent::__construct();
		$this->dir = base_url( 'verivali/qc_foto_ruta_korcam/' );
	...
	function show() {
		...
		$data['grid'] = [
			...
			'toolbars' => "
				...
				{ separator: true },
				{ display:'Approve', name:'approve', bclass:'publish', onpress:act_selected12, hide:false, urlaction: '" . $this->dir . "act_show' },
				{ separator: true },
				{ display:'Reject ke Status 11k', name:'reject11k', bclass:'unpublish', onpress:act_selected12, hide:false, urlaction: '" . $this->dir . "act_show' },
				{ separator: true },
		...
		$this->template->content( "general/Table_grid_view_qc_foto_korcam", $data );
		...
		if ( isset( $input['params'] ) ) {
			...
			foreach ( $params[0] as $field => $value ) {
				if ($field == 'id_prelist') {
                    ...
                    else $where["md.stereotype IN ('HASIL-PEMADANAN',
								'VERIVALI-SUPERVISOR-APPROVED') "] = null;
				} else if ( $field == 'is_in'){
			...
			$where["md.stereotype IN ('HASIL-PEMADANAN',
								'VERIVALI-SUPERVISOR-APPROVED') "] = null;
		}
	...
	function act_show(){
			...
			if ( is_array( $arr_id ) ) {
				...
				foreach ($arr_id as $id) {
					$params_get_audit_trail = [
						...
						'select' => 'audit_trails, stereotype, flag_dokumen'
                    ];
                    ...
                    $flag_dokumen = get_data( $params_get_audit_trail )->row('flag_dokumen');

                    $arr_stereotype = ['HASIL-PEMADANAN'];
			        if ( in_array( $stereotype, $arr_stereotype ) && ($flag_dokumen == 1 || $flag_dokumen == 2)) {                        
                        $audit_trails[] = [
                        ...
                        $params_insert_master_data_log = [
                            ...
                            'data_log_description' => 'Data proses id ' . $get_data->proses_id . ' diubah status menjadi VERIVALI-KORCAM-APPROVED',
		...
		// reject 11p
		if ( isset( $in['reject11k'] ) && $in['reject11k'] ) {
			$arr_id = json_decode( $in['item'] );
			if ( is_array( $arr_id ) ) {
				$success = 0;
				$datetime = date( "Y-m-d H:i:s");
				foreach ($arr_id as $id) {
			        ...
			        if ( in_array( $stereotype, $arr_stereotype ) ) {    
                        ...
                        $params_insert_master_data_log = [
                            'data_log_master_data_id' => $get_data->proses_id,
                            'data_log_status' => 'sukses',
                            'data_log_description' => 'Data proses id ' . $get_data->proses_id . ' diubah status menjadi REJECT-KORCAM',
                            'data_log_stereotype' => $get_data->stereotype,
                            'data_log_row_status' => $get_data->row_status,
                            'data_log_lastupdate_by' => $this->user_info['user_id'],
                            'data_log_lastupdate_on' => date( "Y-m-d H:i:s"),
				...
				$arr_output['message'] = $success .' data berhasil diupdate.';
				$arr_output['message_class'] = 'response_confirmation alert alert-success';
			} else {
				$arr_output['message'] = 'Anda belum memilih data.';
				$arr_output['message_class'] = 'response_error alert alert-danger';
			...
			<div class="row" style="font-size:12px;margin-top:-10px;margin-bottom:-10px;">
				...
				<div class="form-group col-md-3">
					<select id="select-status" name="status" class="js-example-basic-single form-control">
						...
						<option value="HASIL-PEMADANAN">11p.Data HASIL PEMADANAN</option>						
						<option value="VERIVALI-SUPERVISOR-APPROVED">12.Data APPROVED KORCAM</option>						
					</select>
				</div>
				<div class="form-group col-md-3">
