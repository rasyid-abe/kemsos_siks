...
// show status 14
class Qc_foto_ruta_korwil extends Backend_Controller {
	public function __construct() {
		parent::__construct();
		$this->dir = base_url( 'verivali/verivali_final/' );
	}
	...
	function show() {
		...
		$data['grid'] = [
			...
			'columns' => "
				...
				{ name:'status', display:'Jenis', width:50, sortable:false, align:'center', datasuorce: false},
		...
		$data['grid']['title'] = 'VERIVALI FINAL';
	...
	function get_show_data(){
		...
		if ( isset( $input['params'] ) ) {
			...
			foreach ( $params[0] as $field => $value ) {
				if ( $field == 'stereotype' || $field == 'row_status' ) {
					if ( $value > '0' ) $where['md.' . $field] = $value;
					else $where["md.stereotype IN ('VERIVALI-SUPERVISOR-APPROVED', 'VERIVALI-KORWIL-REJECTED') "] = null;
				} else {
		...
		foreach ( $query->result() as $par => $row ) {
			...
			$foto_ktp = '<img src="' . $path_foto . '/' . $row->foto_ktp . '">';
			$foto_kk = '<img src="' . $path_foto . '/' . $row->foto_kk . '">';
			$foto_responden = '<img src="' . $path_foto . '/' . $row->foto_kk1 . '">';
			$foto_rumah = '<img src="' . $path_foto . '/' . $row->foto_rumah . '">';
			$foto_atap = '<img src="' . $path_foto . '/' . $row->foto_kk2 . '">';
			$foto_lantai = '<img src="' . $path_foto . '/' . $row->foto_kk3 . '">';
			$foto_dinding = '<img src="' . $path_foto . '/' . $row->foto_kk4 . '">';
			$foto_dapur = '<img src="' . $path_foto . '/' . $row->foto_kk5 . '">';
			$foto_kloset = '<img src="' . $path_foto . '/' . $row->foto_kk6 . '">';
			$detail = '<button class="btn btn-warning btn-sm"><i class="fa fa-list"></i><button>';
			$informasi_data = '
				<span>
					Kecamatan: ' . $row->district_name . '
					Kelurahan: ' . $row->village_name . '
					Nama KRT: ' . $row->nama_krt . '
					Kelamin KRT: ' . ( ( $row->jenis_kelamin_krt == "1" ) ? "1 = Laki-Laki" : "2 = Perempuan" ) . '
					NIK KRT: ' . $row->nomor_nik . '
					No. KK KRT: 3271040606070609

					Hasil Musdes: 2 = Tidak Ditemukan
					Keluraga Mampu:
					Hasil Verivali:
					...
					Jumlah ART: 3
					Jumlah NIK: 2
					Jumlah Keberadaan ART: 0
					Total Foto: 0
					Jumlah Anak Dalam Tanggungan: 0
					Jumlah Usaha ART: 0

					Jumlah ART Status KRT: 0
					Jumlah ART Status KK: 1
					Jumlah ART Status Istri: 1
					Jumlah ART Status Anak: 1
					Jumlah Nama Ibu Kandung: 0

					Peserta KKS/KPS: 0
					Peserta KIS/PBI JKN: 0
					Peserta KIP/BSM: 0
					Peserta PKH: 0
					Peserta Raskin/Rastra: 0

					Ruta Peserta KKS/KPS:
					Ruta Peserta KIP/BSM: 1 = Ya
					Ruta Peserta KIS/PBI JKN:
					Ruta Peserta BPJS Mandiri:
					Ruta Peserta Jamsostek:
					Ruta Peserta Asuransi:
					Ruta Peserta PKH:
					Ruta Peserta Raskin/Rastra:
					Ruta Peserta KUR:
	...
	function form_cari() {
		...
		$form_cari = '
			<div class="row">
				...
				<div class="form-group col-md-3">
					<label>Jenis</label>
					<select id="select-jenis" name="jenis" class="form-control">
