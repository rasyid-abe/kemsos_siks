...
class Monev_musdes extends Backend_Controller {
	public function __construct() {
		parent::__construct();
		$this->dir = base_url( 'monev/monev_musdes/' );
	}
	...
	function show() {
		...
		$data['grid'] = [
			...
			'columns' => "
				...
				{ name:'jam_mulai_musdes', display:'Jam Mulai', width:80, sortable:false, align:'center', datasuorce: false},
				{ name:'jam_selesai_musdes', display:'Jam Selesai', width:80, sortable:false, align:'center', datasuorce: false},
				{ name:'lokasi_mnv_musdes', display:'Lokasi Musdes', width:170, sortable:true, align:'left', datasuorce: false},
				{ name:'jumlah_rw', display:'Jumlah RW', width:120, sortable:true, align:'center', datasuorce: false},
				{ name:'jumlah_rt', display:'Jumlah RT', width:120, sortable:false, align:'center', datasuorce: false},
				{ name:'bintara_pembina_desa', display:'Pembina Desa', width:120, sortable:true, align:'center', datasuorce: false},
				{ name:'tokoh_masyarakat', display:'Tokoh Masyarakat', width:120, sortable:true, align:'center', datasuorce: false},
				{ name:'tokoh_agama', display:'Tokoh Agama', width:120, sortable:true, align:'center', datasuorce: false},
				{ name:'aparat_desa', display:'Aparat Desa', width:120, sortable:true, align:'center', datasuorce: false},
			...
			",
			'filters' => "
				{ display:'Petugas Monev', name:'mb.petugas_mnv_musdes', type:'text', isdefault: true },
			",
		];
	...
	function get_show_data() {
        ...
        $sql_query = "
            SELECT
                mb.monev_musdes_id,
                mb.tanggal_mnv_musdes,
                mb.jam_mulai_musdes,
                mb.jam_selesai_musdes,
                mb.lokasi_mnv_musdes,
                mb.jumlah_rw,
                mb.jumlah_rt,
                mb.bintara_pembina_desa,
                mb.tokoh_masyarakat,
                mb.tokoh_agama,
                mb.aparat_desa,
                mb.lastupdate_on,
                ...
                l.district_name,
                l.village_name,
                mb.petugas_mnv_musdes AS petugas_monev
            FROM monev.monev_musdes mb
		...
		// $params_count = [
		// 	'table' => [
		// 		'monev.monev_musdes mb' => '',
		// 		'dbo.ref_locations l' => ['mb.location_id = l.location_id', 'left'],
		// 		'dbo.core_user_profile u' => ['mb.created_by = u.user_profile_id', 'left'],
		// 	],
		// 	'select' => 'count(monev_musdes_id) jumlah',
		// 	'where' => $where,
		// ];
		...
		foreach ( $query->result() as $par => $row ) {
			...
			$detail = '<button act="' . base_url( 'monev/monev_musdes' ) . '/get_form_detail/' . enc( [ 'monev_musdes_id' => $row->monev_musdes_id ] ) . '" class="btn btn-warning btn-xs btn-edit" title="Detail" style="padding: 0.1rem 0.2rem !important;font-size: 0.5rem !important;line-height: 1.5 !important;border-radius: 2px !important;"><i class="fa fa-edit"></i></button>';
			$row_data = [
				'id' => $row->monev_musdes_id,
				'cell' => [
					'province_name' => $row->province_name,
					'regency_name' => $row->regency_name,
					'district_name' => $row->district_name,
					'village_name' => $row->village_name,
					'monev_musdes_id' => $row->monev_musdes_id,
					'tanggal' => $row->tanggal_mnv_musdes,
					'jam_mulai_musdes' => $row->jam_mulai_musdes,
					'jam_selesai_musdes' => $row->jam_selesai_musdes,
					'lokasi_mnv_musdes' => $row->lokasi_mnv_musdes,
					'jumlah_rw' => $row->jumlah_rw,
					'jumlah_rt' => $row->jumlah_rt,
					'bintara_pembina_desa' => $row->bintara_pembina_desa,
					'tokoh_masyarakat' => $row->tokoh_masyarakat,
					'tokoh_agama' => $row->tokoh_agama,
					'aparat_desa' => $row->aparat_desa,
	...
	function get_form_detail( $par = null ) {
		...
		$monev_musdes_id = $params['monev_musdes_id'];
		$data = [];
		$data['monev_musdes_id'] = $params['monev_musdes_id'];
		$data['monev_detail'] = $this->get_monev_musdes_information( $monev_musdes_id );
		...
		$params_foto = [
			...
			'where' => [
				'owner_id' => $monev_musdes_id,
				"files.stereotype like 'MM-%'" => null,
			],
		];
		$query_foto = get_data( $params_foto );
		$data['foto'] = $query_foto->result();
		$this->load->view("monev/detail_monev_musdes", $data);
	}

	function get_monev_musdes_information( $monev_musdes_id ){
		$params_user = [
			'table' => [
				'monev.monev_musdes mb' => '',
				'dbo.ref_locations l' => ['mb.location_id = l.location_id', 'left'],
			...
			'where' => [
				'mb.monev_musdes_id' => $monev_musdes_id
			]
		];
